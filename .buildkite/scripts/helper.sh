...
run_workflow() {
  ...
  if [[ "${CREATE_PR:-true}" == "true" ]]; then
    ...
    local commit_message="${pr_title_prefix} ${INTEGRATION} integration
...
Integration: ${INTEGRATION}"
    ...
    local pr_title="${pr_title_prefix} ${INTEGRATION} integration"
    local pr_body="## 🔧 Automated Integration Update
...
AVAILABLE FUNCTIONS:
  Core Functions:
    - run_workflow(setup_func, commands_func)  # Main workflow runner
